...
# 类的命名注意不能重复
class Donchian_Demo(CtaTemplate):
    """1. 唐奇安通道
       2. 日均线判断方向
       3. 自适应出场
    """
    ...
    def on_20min_bar(self, bar: BarData):
        ...
        if self.pos == 0:
            if bar.close_price > self.EntryLong_price and bar.close_price > self.dailyMA_value:
                # 这里用停止单为什么视频里面有讲 但是我没懂…… 2022-11-05
            ...
            if self.pos > 0:
                ...
                if bar.close_price < self.Exit_price:
                    self.sell(bar.close_price, abs(self.pos), True) #平仓
            ...
            if self.pos < 0:
                ...
                if bar.close_price > self.Exit_price:
                    self.cover(bar.close_price, abs(self.pos), True) #平仓

                if bar.close_price > self.EntryLong_price and bar.close_price > self.dailyMA_value:
                    self.buy(bar.close_price, self.fixed_size, True) #开仓
