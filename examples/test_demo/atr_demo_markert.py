    ...
    def on_5min_bar(self, bar: BarData):
        ...
        if self.pos == 0 and self.count_control_num <= self.count_control:
            if bar.close_price > self.up_line :
                self.buy_market(bar.close_price, 1)
                # 开仓了才算交易1次 \
                # 但实际上只是发了一个单子，并不一定成交 \
                # 如果去撮合的话，有可能撮合不成功 \
            ...
            if bar.close_price < self.down_line :
                self.short_market(bar.close_price, 1)
        elif self.pos > 0:
            if bar.close_price < self.down_line:
                self.sell_market(bar.close_price, abs(self.pos))
            ...
            if bar.close_price > self.up_line:
                self.cover_market(bar.close_price, abs(self.pos))
                if self.count_control_num <= self.count_control:
                    self.buy_market(bar.close_price, 1)
            if self.pro_tar:
